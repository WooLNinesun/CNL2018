...
for port in 53 80 443 20 21 23; do
  for potocol in tcp udp; do
    sudo iptables -t filter -A FORWARD -i enp0s3 -o enp0s8 -p ${potocol} --sport ${port} -j ACCEPT
    sudo iptables -t filter -A FORWARD -i enp0s8 -o enp0s3 -p ${potocol} --dport ${port} -j ACCEPT
  done
done
